...
- **Service Layer**: Processing models (TemplateScore, PlanningDocumentGenerator, etc.)
...
#### AgentRecommendations
**Purpose**: Complete classification results and planning guidance
**Location**: `src/types/agent.ts:48-57`
...
interface AgentRecommendations {
  ...
  systemPrompt?: string;                       // Optional customized system prompt
...
**Note**: No code generation - recommendations inform the planning document only
...
#### PlanningDocumentGenerator
**Purpose**: Generates comprehensive planning documents for agent implementation
**Location**: `src/lib/documentation/document-generator.ts`

**Key Methods:**
- `generate(requirements, recommendations)` - Creates complete planning document
- `generateSection(sectionName, context)` - Generates individual sections

**Output Structure:**
Single Markdown code fence with 8 standardized sections:
1. **Overview** - Project summary and goals
2. **Requirements** - Detailed requirement specifications
3. **Architecture** - System architecture and patterns
4. **Implementation Phases** - Step-by-step implementation plan
5. **Security Considerations** - Security best practices
6. **Success Metrics** - Measurement criteria
7. **Risk Mitigation** - Potential risks and mitigation strategies
8. **Deployment Guide** - Deployment and maintenance instructions

**File Header**: `### File: docs/planning.md`

**Copy Instructions**: `**To use**: Copy the above Markdown content...`

#### generate_planning_document (MCP Tool)
**Purpose**: MCP tool handler for planning document generation
**Location**: `src/lib/documentation/planning-tool.ts`
...
**Output**: Markdown code fence containing the complete planning document

**Error Handling**: Returns Markdown-formatted error messages with troubleshooting guidance
...
flowchart TD
    ...
    Recommendations --> Documentation{Documentation Phase}
    Documentation -->|generate_planning_document| DocGen[PlanningDocumentGenerator]

    DocGen --> Markdown[Markdown: planning document]
...
### Removed Code Generation Architecture

**Timeline**: Pre-2025-11-03 → Retired in documentation-first workflow transition

**Previously Removed Modules**:
- `src/lib/generation/` - Code scaffolding generator (retired)
- `src/lib/export/` - File writing and packaging utilities (legacy only, not used in main workflow)

**Deprecated Tools**:
- `generate_agent_code` - TypeScript code scaffolding tool (removed)
- `generate_system_prompt` - System prompt generation tool (removed)
- `generate_config_files` - Configuration file generator (removed)

**Deprecated Type Fields**:
- `AgentRecommendations.starterCode` - Generated TypeScript code field (removed from schema)

**Why Removed**:
1. **Code Scaffolding Drift**: Generated code quickly became outdated as SDK patterns evolved
2. **Strategic Focus**: Advisor should guide planning, not generate implementation artifacts
3. **Toolset Simplification**: Three core tools (interview, classify, document) provide clearer workflow
4. **User Control**: Markdown-only output gives users full control over when/where to persist content

**Architecture Impact**:
- Single Markdown planning document replaces multiple code/config files
- `/save` command for user-controlled persistence replaces automatic file operations
- Documentation-first approach reduces maintenance burden and eliminates drift

**Migration Path**:
- All templates converted to `DocumentTemplate` format with 8 standardized sections
- Planning documents provide implementation guidance without generating code
- Users implement following documented roadmap with full flexibility
...
- Documentation-first planning workflow
