import type { AgentRecommendations, AgentRequirements, AgentTemplate } from '../../types/agent.js';
import { getTemplateById } from '../../templates/index.js';
import { createToolError, ErrorCodes } from '../../types/errors.js';
...
/**
 * Generates the eight-section planning brief consumed by the documentation tool.
 */
export class PlanningDocumentGenerator {
  generate(options: PlanningDocumentOptions): string {
    ...
    if (!template) {
      throw createToolError(ErrorCodes.TEMPLATE_NOT_FOUND, `Template "${templateId}" not found`, {
  ...
  private validateAgentName(agentName: string) {
    ...
    if (!agentName || !validIdentifier.test(agentName)) {
      throw createToolError(ErrorCodes.INVALID_AGENT_NAME, 'Agent name must be at least three characters and start with a letter or underscore.');
    }
  }
